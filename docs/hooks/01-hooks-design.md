# sigma Hook System — Design

> Claude Code / Codex–compatible hook execution for the `sigma` umbrella.
> Status: design proposal. Anchored to current `main` (`sigma_agent`, `sigma_coding`, `sigma_session`).

## 1. Goal & non-goals

**Goal.** Run existing `hooks.json` / `settings.json` workflows from Claude Code and Codex **unchanged** inside `sigma`. A `.codex/hooks.json` or `.claude/settings.json` authored for either CLI fires here with identical stdin payloads, exit-code semantics, and decision schemas.

**Non-goals.**
- Not a plugin framework. Hooks are external `command`/`http` handlers, not Elixir extensions.
- No coupling to other repos (Samgita/Synapsis/backplane). Fully self-contained in `sigma`.
...
2. **The interception point is the existing pipeline, not new call sites.** Tool hooks ride `Sigma.Coding.Dispatcher.do_dispatch`; lifecycle hooks ride `Sigma.Agent`'s `emit` sites. No parallel event bus.
...
|---|---|---|
| `Sigma.Coding.PermissionInterceptor.check/2` | gate before `Tool.execute` | `PreToolUse` |
| `Sigma.Coding.PermissionInterceptor` `:ask` branch | when a permission dialog would show | `PermissionRequest` |
| `Sigma.Coding.Dispatcher.do_dispatch` (post-`Tool.execute`) | result wrap before return | `PostToolUse` |
| `Sigma.Agent` `emit({:agent_start, cwd})` (~ag:295) | session start | `SessionStart` |
| `Sigma.Agent` user-message build (~ag:301) | prompt submitted | `UserPromptSubmit` |
| `Sigma.Agent` `emit({:turn_end, …})` / before `{:agent_end}` (~ag:261/344) | loop wants to stop | `Stop` |
| `Sigma.Agent` `emit({:compact, …})` (~ag:529) | compaction | `PreCompact` |
| `Sigma.Agent.dispatcher_opts` (state field, ag:21/134) | injection channel for specs | — |

`tool_call.name` (`Sigma.Agent.Message` tool-call struct) feeds both the matcher and stdin `tool_name` — **after aliasing to the external Claude/Codex name** (see §3.1). Because `sigma` has real tools, matchers like `Edit|Write` actually fire — a strict **superset** of Codex, which today only emits `Bash`. Existing `"Bash"` matchers keep working.

### 3.1 Tool-name aliasing (hard requirement)

`sigma` tools are lowercase internally (`bash`, `edit`, …); Claude/Codex configs match PascalCase. The matcher and stdin payload use the **external** name; the dispatcher still executes by the internal name.
...
```
                         ┌─────────────────────────────────────────┐
   hooks.json /          │            Sigma.Coding.Hooks (facade)        │
...
### 5.1 stdin (built from `Sigma.Agent` state)
...
- command `cwd` = session `cwd` (`Sigma.Agent` state).
...
1. **`model` field** — Claude emits it only on `SessionStart`; Codex emits it broadly. `sigma` emits on `SessionStart` always and elsewhere as a harmless superset.
...
3. **PreToolUse `defer`** — Claude-only, `-p` mode. `sigma` parses it; if not running headless it degrades to `:ask` with a logged notice.
...
Project-layer specs execute only when the repo is marked trusted (home: `sigma_session/repo_manager.ex`). Untrusted repo → user + plugin layers still run; project layers are loaded-but-skipped with a surfaced notice. This matters more than for a local CLI because the executor is a long-lived server process.
...
- **`updatedInput` / `allow` on PreToolUse.** Codex currently fails-open on these (WIP); `sigma` honors them since it owns the executor. Default behavior documented so configs are predictable.
