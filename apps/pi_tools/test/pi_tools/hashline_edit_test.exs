defmodule PiTools.HashlineEditTest do
  ...
  test "read records a hashline tag that edit uses to replace a line", %{tmp_dir: tmp_dir} do
    ...
    assert {:ok, read_result} =
             PiTools.Read.execute("read", %{"path" => "a.txt"}, cwd: tmp_dir, tool_state: store)
    ...
    assert {:ok, edit_result} =
             PiTools.Edit.execute("edit", %{"input" => input}, cwd: tmp_dir, tool_state: store)
