defmodule Sigma.Web.SessionSupervisorTest do
  ...
  defp session_process_count(repo, session_id) do
    ...
    |> Enum.map(fn role -> Sigma.Agent.Runtime.lookup(repo, session_id, role) end)
  ...
  defp stop_session_supervisor(repo, session_id) do
    case Sigma.Agent.Runtime.lookup(repo, session_id, :supervisor) do
      pid when is_pid(pid) ->
  ...
  test "killing PermissionPolicy brings down the entire session subtree", %{repo: repo} do
    ...
    assert [] = Registry.lookup(Sigma.Agent.RepositoryRegistry, {repo, session_id, :agent})
    assert [] = Registry.lookup(Sigma.Agent.RepositoryRegistry, {repo, session_id, :policy})
  end

  test "killing Sigma.Agent brings down the entire session subtree", %{repo: repo} do
    session_id = unique_session_id()
    handle = start_session(repo, session_id)
  ...
  test "Runtime rebuilds session after subtree crash", %{repo: repo} do
    ...
    {:ok, handle1} = Sigma.Agent.Runtime.get_session(repo, session_id, cwd: repo)
    ...
    {:ok, handle2} = Sigma.Agent.Runtime.get_session(repo, session_id, cwd: repo)
  ...
  test "session supervisor, high-usage compaction, tolerant replay, and crash rebuild interact cleanly",
    ...
    assert [%Sigma.Agent.Message{role: :compaction_summary, content: compact_content} | kept] =
             replayed_messages
