defmodule Sigma.Web.SessionManager do
  @moduledoc """
  Compatibility facade for the repository-owned `Sigma.Agent.Runtime`.
  """
  ...
  def get_agent(session_id, opts \\ []) when is_binary(session_id) do
    ...
    if is_binary(repo_path) do
      case Sigma.Agent.Runtime.get_session(repo_path, session_id, opts) do
