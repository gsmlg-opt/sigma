      ...
      if File.exists?(resolver_path) do
        ...
        content1 =
          if String.contains?(content, "when is_atom(key) -> acc") do
            new = String.replace(content, "when is_atom(key) -> acc", "when not is_binary(key) -> acc")
        ...
        if String.contains?(content2, "defp extract_conflict_package(message) do") do
          ...
          new_content = String.replace(normalized_content, normalized_target, normalized_replacement)
                ...
                {:ok, entry_path, project_root} ->
                  ...
                  static_externals =
                    case File.read(entry_path) do
                      {:ok, source} ->
                        case OXC.imports(source, Path.basename(entry_path)) do
