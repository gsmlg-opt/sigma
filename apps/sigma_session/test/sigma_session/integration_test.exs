defmodule Sigma.Session.IntegrationTest do
  ...
  describe "Crash Test" do
    test "agent can resume after a crash" do
      ...
      {:ok, agent1} = Sigma.Agent.start_link(model: model, provider: MockProvider)
      Sigma.Agent.subscribe(agent1)
  ...
  describe "Branch Test" do
    test "forked sessions advance independently" do
      ...
      {:ok, parent_agent} = Sigma.Agent.start_link(model: model, provider: MockProvider)
      Sigma.Agent.subscribe(parent_agent)
      ...
      Sigma.Agent.subscribe(branch_agent)
