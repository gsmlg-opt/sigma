defmodule Sigma.Session.Storage.JsonlFile do
  ...
  defp emit_diagnostics(path, bad_lines, total_lines) do
    ...
    Enum.each(interior, fn {idx, _} ->
      Logger.warning(
        "[Sigma.Session] Skipping corrupt line #{idx} in #{path} (#{bad_count} bad line(s) total)"
      )
    end)

    Enum.each(trailing, fn {idx, _} ->
      Logger.debug(
        "[Sigma.Session] Skipping trailing incomplete line #{idx} in #{path} — likely torn write"
      )
    end)
  end
end
