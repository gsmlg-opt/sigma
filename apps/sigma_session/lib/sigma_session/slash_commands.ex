defmodule Sigma.Session.SlashCommands do
  @moduledoc """
  ...
  Ask the user concise questions in chat. If Sigma Agent exposes a structured user-question tool, use it; otherwise ask normally. Ask only Q1 first. Ask Q2 only after the Q1 answer, because "Let Sigma Agent decide" skips Q2.

  Before the first question, print this primer as normal assistant text:

  > Quick context:
  > - Project AGENTS.md gives Sigma Agent persistent, team-shared instructions for this repository. Sigma Agent reads it when working in this repo.
  > - User AGENTS.md at `~/.pi/agent/AGENTS.md` gives Sigma Agent personal/global instructions outside source control.
  > - Skills are packaged instructions Sigma Agent can discover from `.agents/skills` or `~/.agents/skills` and inject when a task matches, or that the user can trigger by name.
  > - Hooks are deterministic commands on lifecycle events. Only set them up when this repo already has Sigma Agent hook support, or when the user explicitly asks to design that support.
  ...
  - Q1: "Which AGENTS.md files should /init set up?"
    Options: "Project AGENTS.md" | "User AGENTS.md" | "Both project + user" | "Let Sigma Agent decide"
    Description for project: "Team-shared instructions checked into source control: architecture, coding standards, common workflows."
    Description for user: "Your private/global Sigma Agent instructions in `~/.pi/agent/AGENTS.md`: role, preferences, sandbox URLs, workflow quirks."
    Description for Let Sigma Agent decide: "Fastest path: project AGENTS.md plus whatever skills or supported hooks fit this repo. No follow-on setup-scope questions; you will approve everything before it is written."
    If the user picks "Let Sigma Agent decide", skip Q2 and treat it as project AGENTS.md with no hard skills/hooks constraint.
  ...
  - Q2: "Also set up skills or supported hooks?"
    ...
    Description for skills: "Packaged instructions Sigma Agent can discover and inject for repeatable workflows and reference knowledge."
    Description for hooks: "Deterministic shell commands that run on tool events. Only create hook config if this repo already supports Sigma Agent hooks or the user explicitly asks to add support."
    Q2 is a hint, not a hard filter. Phase 3 proposes what fits the repo and notes any deviation.

  ## Phase 2: Explore the codebase

  Survey the codebase. If Sigma Agent has parallel subagents available, use one for the survey; otherwise inspect directly. Read key files that explain the project: manifest files (`mix.exs`, `package.json`, `Cargo.toml`, `pyproject.toml`, `go.mod`, `pom.xml`, etc.), README, Makefile/build configs, CI config, existing AGENTS.md, `.agents/skills/`, existing hook config, other agent instructions, and MCP config.
  ...
  If the user chose project AGENTS.md, both, or "Let Sigma Agent decide", ask about codebase practices: non-obvious commands, gotchas, branch/PR conventions, required env setup, and testing quirks. Skip anything already in README or obvious from manifest files. Do not mark any option as recommended; this is about how their team works.
  ...
  - Do they have personal sandbox URLs, test accounts, API key paths, or local setup details Sigma Agent should know?
  ...
  - Hook: deterministic, fast, per-edit or lifecycle shell command, only when Sigma Agent hook support exists or is explicitly requested.
  ...
  Write a minimal AGENTS.md at the project root. Every line must pass this test: "Would removing this cause Sigma Agent to make mistakes?" If no, cut it.
  ...
  Include:
  - Build/test/lint commands Sigma Agent cannot reliably guess.
  - Code style rules that differ from language defaults.
  - Testing instructions and quirks.
  - Repo etiquette such as branch, PR, and commit conventions.
  - Required env vars or setup steps.
  - Non-obvious gotchas or architectural decisions.
  - Important content from existing AI coding tool configs when it applies to Sigma Agent.

  Exclude:
  - File-by-file structure or component lists that Sigma Agent can discover by reading the codebase.
  - Standard language conventions Sigma Agent already knows.
  ...
  This file provides guidance to Sigma Agent when working with code in this repository.
  ```
  ...
  Write a minimal user AGENTS.md at `~/.pi/agent/AGENTS.md`. This file is outside source control and applies globally to Sigma Agent sessions.

  Consume approved `note` entries whose target is user AGENTS.md. Include only what would make Sigma Agent's responses noticeably better for this user:
  ...
  Skills add capabilities Sigma Agent can use on demand without bloating every session.
  ...
  - Linting: if Phase 2 found no lint config for the project's language, ask whether the user wants Sigma Agent to set one up.
  - Supported hooks: only consume approved hook entries when this repo already has Sigma Agent hook support or the user explicitly approved adding it. Do not create hook config for another agent runtime.
  - Missing or sparse tests: suggest a focused test setup so Sigma Agent can verify its own changes.
